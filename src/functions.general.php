...
HTML;
        ...
        if (isset($button['coins']) && $button['coins'] > 0) {
            $content .= '<img src="img/x.gif" class="goldIcon" alt="" /><span class="goldValue">' . $button['coins'] . '</span>';
...
{
    $version = 'v1';
  	$fileName = 'crypt.js?' . $version;
    if (is_lowres()) {
        $fileName = 'crypt-lowres.js?' . $version;
    }
    return $fileName;
}
...
function get_gpack_link_and_hash($fileName, $useLang = true)
{
    if ($useLang) {
        ...
        $fileName = get_gpack_cdn_mainPage_url() . 'lang/' . $fileName;
    }
    return $fileName;
}
...
function is_my_ip()
{
    return WebService::ipAddress() == '88.99.108.8';
}
