// ==UserScript==
// @name           Travian Resource bar plus (Fixed for Private Servers)
// @namespace      https://github.com/adipiciu/Travian-scripts
// @include        *://*.travian.*
// @include        *://localtrav.test/*
// @include        *://*.localtrav.test/*
// @include        *://10x.localtrav.test/*
// @grant          GM_getValue
// @grant          GM_setValue
...
(function () {
    ...
    function allInOneOpera() {
        ...
        var langs = ['auto', 'English (en)', 'Nederlands (nl)'];
        ...
        function $gc(cls, root) {
            const target = root || document;
            const collection = target.getElementsByClassName(cls);
            if (!collection || collection.length === 0) {
                return []; // Return empty array instead of null to prevent [0] errors
            }
            return collection;
        }

        // Safer XPath selector
        function $xf(path, type, root) {
            try {
                const target = root || document;
                const res = document.evaluate(path, target, null,
                    type === 'l' ? XPathResult.ORDERED_NODE_SNAPSHOT_TYPE : XPathResult.FIRST_ORDERED_NODE_TYPE, null);
                ...
                console.error(`[TTQ Debug] XPath Error for path [${path}]:`, e);
                return null;
            }
        }
        ...
        function getUserID() {
            // 1. Read from meta tag injected by server (most reliable)
            var meta = document.querySelector('meta[name="player-id"]');
            if (meta) {
                var mid = parseInt(meta.content);
                if (mid > 0) { window.localStorage.setItem('TRBP_cached_uid', mid); return mid; }
            }

            // 2. Try Travian game object (window fallback for private servers)
            var tw = (typeof unsafeWindow !== 'undefined') ? unsafeWindow : window;
            if (tw.Travian && tw.Travian.Game && tw.Travian.Game.player) {
                var gid = parseInt(tw.Travian.Game.player.id);
                if (gid > 0) { window.localStorage.setItem('TRBP_cached_uid', gid); return gid; }
            }

            // 3. Try profile link in HTML
            var profileLink = document.querySelector('a[href*="spieler.php?uid="], a[href*="profile/"]');
            if (profileLink) {
                var match = profileLink.href.match(/uid=(\d+)/) || profileLink.href.match(/profile\/(\d+)/);
                if (match) {
                    var id = parseInt(match[1]);
                    window.localStorage.setItem('TRBP_cached_uid', id);
                    return id;
                }
            }

            // 4. Fallback to cache
            var cached = parseInt(window.localStorage.getItem('TRBP_cached_uid') || -1);
            ...
            for (var vn = 0; vn < villages_count; vn++) {
                ...
                for (var i = 0; i < 4; i++) {
                    ...
                    if (i > 1) {
                        ...
                        if (Math.abs(minLeft) < 600) setInterval(function (x) {
                            return function () {
        ...
        function parseDorf1() {
            ...
            function detectTribe() {
                ...
                if (typeof unsafeWindow.Travian !== 'undefined' && unsafeWindow.Travian.Game.Tribe) {
                    race = parseInt(unsafeWindow.Travian.Game.Tribe);
                } else {
                    // Fallback: Default to Huns (6) if playing on a Huns-enabled server and detection fails
                    race = parseInt(RB_getValue(GMcookieID + 'race', 6));
                }
                return race;
            }
        ...
        function detectAttack() {
            ...
            function findAttackN() {
                var aLink = fullName + 'dorf1.php';
                ajaxRequest(aLink, 'GET', null, function (ajaxResp) {
                    var ad = ajaxResp.responseText;
                    var reg = new RegExp('"gray":([1-9])');
                    if (reg.test(ad)) {
                        triggerAlarm();
                        return true;
                    } else { noAttack(); }
        ...
        if (/^\/profile(?!\?)/.test(relName) || /spieler\.php/.test(relName)) {
        ...
        const mutationCallback = (mutationsList, observer) => {
            for (const mutation of mutationsList) {
                if (mutation.type === 'childList') {
                    mutation.addedNodes.forEach(node => {
                        if (node.nodeType === Node.ELEMENT_NODE) {
                            if (node.getAttribute("id") == 'sidebarBoxActiveVillage' || node.getAttribute("id") == 'sidebarBoxVillagelist') {
                                if (RB.Setup[16] != 0) { bigQuickLinks(); }
                                villageHintEdit();
                                vlist_addButtonsT4();
                            }
